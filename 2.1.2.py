...
class report:
    def __init__(self, vacancies_count_by_years, vacancies_count_by_years_for_profession, salary_by_years,
                 salary_by_years_for_profession, vacancies_count_by_cities, vacancies_share_by_cities, salary_by_cities, profession):
    ...
    def fill_column_by_years(self, ws, column_index, dictionary_values, min_year, max_year):
        ...
        for row, value in zip([index for index in range(2, max_row_index + 2)], [dictionary_values[value] for value in range(min_year, max_year)]):
    ...
    def fill_column_by_cities(self, ws, column_index, max_row_index, dictionary_values, keys):
        for row, value in zip([index for index in range(2, max_row_index + 2)], [dictionary_values[key] for key in keys]):
            ws.cell(row=row, column=column_index, value=value)
    ...
    def make_ws_by_years(self, ws):
        ...
        self.make_titles(max_column_index,
                         ["Год", "Средняя зарплата", f"Средняя зарплата - {self.profession}",  "Количество вакансий", f"Количество вакансий - {self.profession}"],
                         ws)
        self.fill_column_by_years(ws, 1, {x: y for x, y in zip(range(min_year, max_year + 1), range(min_year, max_year + 1))}, min_year, max_year + 1)
    ...
    def generate_image(self):
        ...
        self.generate_normal_bar_graph("Количество вакансий по годам", "Количество вакансий", f"Количество вакансий {self.profession}",
...
report(dataset.vacancies_count_by_years, dataset.vacancies_count_by_years_for_profession, dataset.salary_by_years,
       ...
       dataset.vacancies_count_by_cities, dataset.vacancies_share_by_cities, dataset.salary_by_cities, dataset.profession).generate_image()
